 ...
 */
public class IndependentCurrentSource extends CircuitElement {
    double lines[][] =  {
                         {-3,  0, -3,  0},
                         {-2,  0,  2,  0},
                         { 1, -1,  2,  0},
    ...
    public IndependentCurrentSource(Node p, Node n) {
        connections.add(p);
        connections.add(n);
        x = (p.getX() + n.getX()) / 2;
        y = (p.getY() + n.getY()) / 2;
    ...
    public void draw(Graphics g) {
        ...
        if (p != null && n != null) {
            ...
            g.drawString(name + " : " + current + " amperes", (int) (x - width / 2), (int) (y - height / 2));
